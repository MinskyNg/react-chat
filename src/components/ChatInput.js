        ...
        return (
            <div className="content-send">
                <div
                  ...
                  contentEditable="true" placeholder="说些话吧......"
                  ref={ div => this._inputMsg = div }
                >
                </div>
                <button className="button-send" onClick={ () => this.handleClick() }>
                  发送
                </button>
                <span className="info-send">你好 <span className="sender">
                { this.props.user === 'NOT_LOGGED_IN' ? '未登录用户' : this.props.user }
                </span> , 你正在和 <span className="receiver">
                { this.props.receiver === 'SEND_TO_ALL' ? '所有人' : this.props.receiver }
                </span> 聊天</span>
